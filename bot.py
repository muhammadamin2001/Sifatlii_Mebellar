...
async def subcategory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(name, callback_data=f"sub_{code}")] for name, code in cat["items"]]
    keyboard.append([InlineKeyboardButton("🔙 Orqaga", callback_data="catalog")])
    await query.edit_message_text(
        f"📦 *{cat['name']}* — mahsulot turini tanlang:",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    return SUBCATEGORY
...
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    summary = (
        ...
        f"🛋 Mahsulot: *{SUB_NAMES.get(ud['subcategory'], ud['subcategory'])}*\n"
        f"📐 O'lcham: *{ud['size']} sm*\n"
        f"🎨 Rang: *{COLOR_LABELS.get(ud['color'], ud['color'])}*\n"
        f"🪵 Material: *{MAT_LABELS.get(mat_val, mat_val)}*\n\n"
...
async def place_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    order_id = await db.save_order(
        user_id=user.id,
        product=SUB_NAMES.get(ud["subcategory"], ud["subcategory"]),
        size=ud["size"], color=ud["color"],
        material=ud["material"], price=ud["final_price"]
    )
    await query.edit_message_text(
